app.spaces.eventsource = (a,x) => a['app-spaces-eventsource'](
  null,
  {
    $digest: function(e) {
      let serializedData = e.data
      console.info( `Events stream ${ this.$started } - Event data: '${ serializedData }'` )
      if ( serializedData ) {
        ...
        if ( data.type == 'timeout' ) {
          ...
          this.$send( 'app.spaces.status.update', { detail: spaces_status_update } )
        }
      }
    },
    $run: function() {
