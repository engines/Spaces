...
Spaces::Commands::Saving.new(
  model: {repository: 'https://github.com/v2Blueprints/owncloud'},
  space: :locations
).run.payload

# import an application blueprint
Publishing::Commands::Importing.new(identifier: :owncloud, force: true).run.payload
...
# # export a blueprint
# Publishing::Commands::Exporting.new(identifier: :owncloud, message: nil).run.payload

# # get the status of a publication
# Spaces::Commands::Status.new(identifier: :owncloud, space: :publications).run.payload

# synchronize a blueprint
Blueprinting::Commands::Synchronizing.new(identifier: :owncloud).run.payload

# synchronize a publication
Publishing::Commands::Synchronizing.new(identifier: :owncloud).run.payload

# bind another blueprint to the arena
Arenas::Commands::Binding.new(identifier: :docker_arena, blueprint_identifier: :owncloud).run.payload
...
# Spaces::Commands::Validating.new(identifier: 'docker_arena::owncloud', space: :resolutions).run.payload

# # get the status of a resolution
# Spaces::Commands::Status.new(identifier: 'docker_arena::owncloud', space: :resolutions).run.payload
...
Packing::Commands::Saving.new(identifier: 'docker_arena::owncloud').run.payload
...
# Packing::Commands::Artifacts.new(identifier: 'docker_arena::owncloud').run.payload

# provision the arena for applications
Arenas::Commands::Provisioning.new(identifier: :docker_arena).run.payload

# # commit a pack
# Packing::Commands::Executing.new(identifier: 'docker_arena::owncloud', execute: :commit).run.payload
#
...
# Provisioning::Commands::Saving.new(identifier: 'docker_arena::owncloud').run.payload
