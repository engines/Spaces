...
bp_name=:owncloud
bp_uri="https://github.com/v2Blueprints/owncloud"
...
controllers.publishing.import(model: {repository: bp_uri})
...
# blueprint gets
controllers.blueprinting.show(identifier: bp_name)
controllers.blueprinting.summary(identifier: bp_name)
...
controllers.blueprinting.synchronize(identifier: bp_name)

# synchronize a publication to a blueprint
controllers.publishing.synchronize(identifier: bp_name)

# ------------------------------------------------------------------------------

# bind another blueprint to the arena
controllers.arenas.bind(identifier: :docker_arena, blueprint_identifier: bp_name)
...
# Spaces::Commands::Validating.new(identifier: "docker_arena::#{bp_name}", space: :resolutions).run.payload

# save a pack for a resolution
controllers.packing.new(identifier: "docker_arena::#{bp_name}")
...
# controllers.packing.artifacts(identifier: "docker_arena::#{bp_name}")

# provision the arena for applications
controllers.arenas.provision(identifier: :docker_arena)

# # commit a pack
# controllers.packing.commit(identifier: "docker_arena::#{bp_name}")
#
# # apply provisions for an arena
# controllers.arenas.apply(identifier: :docker_arena)

# # save provisions for a resolution
# controllers.provisioning.new(identifier: "docker_arena::#{bp_name}")

# capture registry entries for an application
controllers.registry.register(identifier: "docker_arena::#{bp_name}")
